...
from ros2_rerout import reconnect_omnigraph
...
def setup_custom_env():
    try:
        ...
        if (args_cli.custom_env == "office" and args_cli.terrain == 'flat'):
            ...
            spawn_coords = [[-1.78,-1.21,0], [-1.57,0.4,0], [-1.35,1.9,0], [-0.6,1.35,0], [-0.9,-0.2,0], [-1.23, -2.2,0], [-1.0, -3.3,0], [-0.2, -0.5,0], [0.3, 3.3,0]]

            cfg_item = sim_utils.UsdFileCfg(usd_path="http://omniverse-content-production.s3-us-west-2.amazonaws.com/Assets/ArchVis/Industrial/Containers/Cardboard/Cardbox_D3.usd", scale=(0.01,0.01,0.01))
            for sp_idx,sp in enumerate(spawn_coords):
                cfg_item.func("/World/cardbox" + str(sp_idx), cfg_item, translation=sp) #, scale=(0.01,0.01,0.01))
    except:
...
def add_cmd_sub(num_envs):
    ...
    for i in range(num_envs):
        node_test.create_subscription(Twist, f'cmd_vel', lambda msg, i=i: cmd_vel_cb(msg, str(i)), 10)
